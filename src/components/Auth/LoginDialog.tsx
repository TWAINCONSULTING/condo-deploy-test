    ...
    <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full">
        <div className="relative p-6">
          ...
          <Link
            to=""
            className="mt-4 flex items-center gap-2 text-xs text-gray-500 hover:text-blue-600 transition-colors"
            onClick={handleClick}
          >
            <Bell className="h-3 w-3" />
            <span>Administrer varslingsinnstillinger</span>
            {showKommer && <span className="text-blue-600 font-medium">Kommer!</span>}
          </Link>
        </div>
      </div>
    </div>
